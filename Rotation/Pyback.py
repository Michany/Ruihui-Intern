...
class Backtest:
    ...
    def __init__(self, start_date="2008-01-01", end_date="2018-08-24", 
                *stock_pool, **arg):
    ...
    def loop(self):
        ...
        def _init_loop():
            ...
            self.average_cost, self.investment = pd.DataFrame(), pd.DataFrame()
        ...
        def Sell(**arg):
            ...
            if percentage is None:
                ...
                print(
                    day.strftime("%Y-%m-%d"),
                    "卖出 {:.2f} 份 {}".format(amount, symbol),
                    "({:.1%} 持仓)".format(percentage),
                    "卖价", price,
                )

            share_today[symbol] = self.share[-1][symbol] - amount
            ...
            if share_today[symbol] == 0:
                ...
                self.平均成本[symbol] = (self.平均成本[symbol] * self.share[-1][symbol] - amount * price) / share_today[symbol]
            self.累计投入[symbol] *= 1 - percentage
            return "卖出成功"
        ...
        def Buy(**arg):
            ...
            profit_today[day] = (
                profit_today.get(day, 0)
                + self._price_diff.loc[day, symbol] * self.share[-1][symbol]
            )
        ...
        for day in self.price.index[self.DURATION:]:
            ...
            for symbol in self.STOCK_POOL:
                ...
                self.investment.loc[day, symbol] = self.累计投入[symbol]

            # 记录当日份数和账户余额
            self.share.append(share_today)
            self.capital_balance.append(balance_today)
        t1 = time.time()
        tpy = t1 - t0
