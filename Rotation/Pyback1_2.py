...
class Backtest():
    ...
    def price(self, price_DataFrame):
        if isinstance(price_DataFrame, pd.DataFrame):
            ...
            self.refresh_TI(TI_from_TA_lib={'RSI':(20,)}) #给price赋值时，重新计算技术指标
    ...
    def timing(self):
        # %% 策略部分 分配仓位
        self.RSI=self.RSI.replace(0,np.nan)
            ...
            for symbol in self.POOL:
                ...
                if self.pos.loc[day, symbol] < self.share[-1][symbol]:
                    SELL_SIGNALS[symbol]["按周调仓 卖出"] = {
                        "symbol": symbol,
                        "amount": self.share[-1][symbol]-self.pos.loc[day, symbol],
                        "price": price_today[symbol],
                    }

                if self.pos.loc[day, symbol] > self.share[-1][symbol]:
                    BUY_SIGNALS[symbol]["按周调仓 买入"] = {
                        "symbol": symbol,
                        "amount": self.pos.loc[day, symbol]-self.share[-1][symbol],
                        "price": price_today[symbol],
                    }
...
if __name__ == "__main__":
    ...
    # pool = ["000036.SH"]
    test = Backtest(pool=pool, type='index', duration = 250,
