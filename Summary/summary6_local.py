...
def read_files(filepath:str, symbol = '', Type = 'index'):
    '''
    ...
    f 为文件路径
    '''
    global t, w, f, folder, global_symbol, global_type
    ...
    if symbol=='': print('Warning: symbol missing\nWill try symbol found in input file...')
    if Type=='future':
...
def output_to_excel():
    ...
    def 每日浮动收益统计(symbol='sh000905', Type='stock'):
        import tushare as ts
        
        start_date = str(date_list[0])[:10]
        end_date = str(date_list[-1])[:10]
        print('Fetching data ---',start_date, end_date)
        close = ts.get_hist_data(code='sh000905', start=start_date, end=end_date, ktype='D',retry_count=3, pause=0.001)
        print('Data downloaded.')
        close = close.reindex(close.index.astype('datetime64'),copy=False)
        close.sort_values(by='date',ascending=True, inplace=True)
    ...
    def beautify_excel():
        ...
        def generate_profit_curve():
            ...
            plt.grid()  
            fig.savefig(folder+f+': Daily Profit.png', dpi=100)
...
if __name__ == '__main__':
    read_files(r"E:\RH\excel\500.xlsx", symbol = '', Type='index') 
    start()
    output_to_excel()
