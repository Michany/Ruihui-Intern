# -*- coding: utf-8 -*-
"""
Created on Thu Aug  2 10:17:37 2018
...
class LSTM(nn.Module):
    ...
    def initHidden(self):
        ...
        return (torch.randn(LAYER,BATCH_SIZE,self.input_size).cuda(),torch.randn(LAYER,BATCH_SIZE,self.hidden_size).cuda())
    def pre_hidden(self):
        
        return (torch.randn(LAYER,1,self.input_size).cuda(),torch.randn(LAYER,1,self.hidden_size).cuda())
...
ta_index['close'] = ta_index.sclose
t = (ta_index.iloc[:,:-2] - ta_index.iloc[:,:-2].cummin()) / (ta_index.iloc[:,:-2].cummax() - ta_index.iloc[:,:-2].cummin())
ta_index.iloc[:,:-2] = t

ta_index = ta_index.iloc[100:]
ta_index
ta_index.iloc[1]
def train_data_collecter(ta_index):
    ...
    for i in range(TIME_SERIES,TRAIN_SIZE):
        x = ta_index.iloc[i-TIME_SERIES:i,:-2].values
        x = x.reshape(TIME_SERIES,-1)
        train_xx.append(x)
        train_y.append(ta_index.iloc[i,-2])
...
train_loader,AE_loader = train_data_collecter(ta_index)
...
autoencoder = AutoEncoder(32,15)
autoencoder = autoencoder.cuda()
def AE_featrure():
    
    loss_fn = nn.MSELoss()
    opt = optim.Adam(autoencoder.parameters(),lr=0.01)
    ...
    for epoch in range(200):
        ...
        for orgin_data,data in AE_loader:
            ...
            opt.step()
        if epoch % 10 == 0:
            print("AutoEncoder cycle "+str(epoch)+" done. The mean loss is "+str(total_loss/(TRAIN_SIZE-TIME_SERIES)))
            tim.toc()
            tim.tic()
...
def do_predict(epoch):
    ...
    for i in range(TRAIN_SIZE,len(ta_index)-1):
        Return = ta_index.close.iloc[i+1]/ta_index.close.iloc[i]
        mylstm.hidden = mylstm.pre_hidden()
        x = torch.from_numpy(ta_index.iloc[i-TIME_SERIES:i,:-2].values)
        x = x.contiguous().view(1,TIME_SERIES,-1)
        x = x.cuda()
        x = decoded_layer(x.float())
        out = mylstm(x.float())
        ...
        if signal == 4:
            ...
            short_val.append(short_val[-1])
    benchmark = ta_index.close.iloc[TRAIN_SIZE:]/ta_index.close.iloc[TRAIN_SIZE]
...
def do_train(epoch):
    ...
    for i in range(TIME_SERIES,TRAIN_SIZE-1):
        ...
        if signal == 4:
            ...
            long_short.append(long_short[-1])
            short_val.append(short_val[-1])
    benchmark = ta_index.close.iloc[TIME_SERIES:TRAIN_SIZE]/ta_index.close.iloc[TIME_SERIES]
...
mylstm = LSTM(15,15,5)
mylstm = mylstm.cuda()
loss_fn = nn.CrossEntropyLoss(weight=torch.tensor([3,2,1,2,3]).float().cuda())
