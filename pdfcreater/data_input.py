...
def input_SOdata(DBname,table_name,df,order='fail',chinesecol=[]):
    #order取值说明
    #'fail'：如果表格存在则报错
    #'replace'：替换已经存在表格
    ...
    if table_name in list(tablelist.iloc[:,0]):
        ...
        return 'you cannot replace rawdata'
    else:
        df.to_sql(table_name,engine,if_exists=order,chunksize=1000,index=False,dtype = dtypedict)
        return 'succeed!'
...
def delete_table(DBname = 'StrategyOutput',table_name = 'mhqtest',safe_order = 'on' ):
    ...
    if safe_order == 'on':
        safe_data = get_SOdata(DBname, table_name)
        safe_data.to_csv(table_name+'.csv',index=False)
    sql = 'drop table [%s].[dbo].[%s]'%(DBname,table_name)
    engine.execute(sql);
    return 'succeed!'

def del_rowORcolumns(DBname,table_name,limit,axis = 0 ):
...
def view_tabletime(DBname,table_name,days='20'):
    ...
    if days == 'all':
        ...
        sqlt = "select top %d  * from %s.dbo.%s Order By %s Desc"%(days,DBname,table_name,data[(data.type=='datetime')]['name'].tolist()[0])
        timetable = pd.read_sql(sqlt,con)
        timeline = timetable[tline_name]
    return timeline
